...
import me.limeglass.scoreboardtags.utils.annotations.DetermineSingle;
import me.limeglass.scoreboardtags.utils.annotations.Events;
import me.limeglass.scoreboardtags.utils.annotations.Multiple;
import me.limeglass.scoreboardtags.utils.annotations.Settable;
import me.limeglass.scoreboardtags.utils.annotations.Single;

public abstract class ScoreboardTagsExpression<T> extends SimpleExpression<T> implements DataChecker {
	...
	public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean isDelayed, ParseResult parser) {
		if (getClass().isAnnotationPresent(Events.class)) {
			if (!contains()) {
				ScoreboardTags.debugMessage("The expression `" + getClass().getSimpleName() + "` can't be used in the event: " + ScriptLoader.getCurrentEventName() + "it can only be used in: " + Arrays.toString(getClass().getAnnotation(Events.class).value()));
				return false;
			}
		}
	...
	public String toString(Event event, boolean debug) {
		...
		if (event != null) ScoreboardTags.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
