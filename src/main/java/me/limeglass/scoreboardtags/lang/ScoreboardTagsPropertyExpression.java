...
public abstract class ScoreboardTagsPropertyExpression<F, T> extends PropertyExpression<F, T> {
	...
	public String toString(Event event, boolean debug) {
		...
		return ScoreboardTags.getNameplate() + getClass().getSimpleName() + ": the " + getPropertyName() + " (of|from) " + getExpr().toString(event, debug);
	}
	...
	protected Boolean isNull(Event event) {
		...
		if (getExpr().isSingle() && getExpr().getSingle(event) == null) {
			ScoreboardTags.debugMessage("The property expression was null: " + getExpr().toString(event, true));
			return true;
