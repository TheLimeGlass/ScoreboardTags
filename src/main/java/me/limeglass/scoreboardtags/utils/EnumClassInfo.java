package me.limeglass.scoreboardtags.utils;
...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public void register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (ScoreboardTags.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				ScoreboardTags.getInstance().getConfiguration("syntax").set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enums);
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(ScoreboardTags.getInstance().getConfig().getString("syntaxDebug"))) ScoreboardTags.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else ScoreboardTags.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		ScoreboardTags.getInstance().getConfiguration("syntax").set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
