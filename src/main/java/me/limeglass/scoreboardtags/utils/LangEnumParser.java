...
import me.limeglass.scoreboardtags.lang.ScoreboardTagsParser;

public class LangEnumParser<T extends Enum<T>> extends ScoreboardTagsParser<T> {
	...
	public LangEnumParser(String variableNamePattern, Class<T> clazz) {
		...
		try {
			...
			ScoreboardTags.getInstance().getConfiguration("syntax").set("Syntax.Enums." + clazz.getSimpleName() + ".names", enumNames);
		} catch (NullPointerException error) {
			ScoreboardTags.consoleMessage("&cThe class: " + clazz.getName() +  " for classinfo name: " + variableNamePattern + " isn't an Enum!");
		}
